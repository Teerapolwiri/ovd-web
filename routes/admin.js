...
router.get('/staff', async (req, res) => {
    try {
        const [rows] = await db.query(
            'SELECT id, name, nickname, email, phone, bank_account AS bankAccount, bank_name AS bankName, role, status, registered_at AS registeredAt FROM staff WHERE role != "admin" ORDER BY registered_at DESC'
        );
        res.json({ staff: rows });
    } catch (err) {
...
// ─────────────────────────────────────────────
// POST /api/admin/sessions/:id/paid — toggle จ่ายเงิน
// ─────────────────────────────────────────────
router.post('/sessions/:id/paid', async (req, res) => {
    try {
        const { isPaid } = req.body;
        await db.query('UPDATE sessions SET is_paid = ? WHERE id = ?', [isPaid ? 1 : 0, req.params.id]);
        res.json({ ok: true });
    } catch (err) {
        res.status(500).json({ error: 'เกิดข้อผิดพลาด' });
    }
